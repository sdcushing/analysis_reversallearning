function CellExplorer_Pipeline_JLK(subj, sessDate, neuralRawDataPath, params)
...
session.general.basePath =  [neuralRawDataPath '\kilosort4']; % Full path
...
session.extracellular.fileName = dir([session.general.basePath '\' sprintf('Neuropixels_%s_%s*',subj,sessDate)]).name;
...
session.extracellular.sr = params.ap_samprate;
session.extracellular.nChannels = params.numChans;
