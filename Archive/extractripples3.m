...
%           'inclposinfo' - 0 or 1, default 1.  if set to 0 then does not
%               get pos info.  AS added 12-15-10.
%
% Outputs:
...
if exist('flist') & ~isempty(flist)
    ...
    if samethreshperday == 1
        ...
        for j = 1:length(flist)
            load(flist{j}); %NJ fixed the issue of loading 1st file repeatedly 6/24/22
%             f = str2num(flist{1}(end-5:end-4));
            f = files(j); %NJ fixed the issue of loading 1st file repeatedly 6/24/22

            % convert the ripple envelope field to double
            temprenv = double(ripple{animal}{date}{f}.data(:,3));
    ...
    for i = 1:length(flist)
        ...
        try
            load(flist{i});
%             sprintf(['extracting ripples from ', flist{i}])
        catch
            sprintf(['files missing for extracting ripples from ', flist{i}])
            continue
        end
        
%         f = str2num(flist{i}(end-5:end-4));
        f = files(i);
        
        % convert the ripple envelope field to double
        renv = double(ripple{animal}{date}{f}.data(:,3));
        
        % smooth the envelope:
        samprate = ripple{animal}{date}{f}.samprate;
        ...
        if (thresh > 0) & any(find(renv<baseline))
            ...
            rip.starttime = ripple{animal}{date}{f}.starttime + rip.startind / samprate;
            rip.endtime = ripple{animal}{date}{f}.starttime + rip.endind / samprate;
            rip.midtime = ripple{animal}{date}{f}.starttime + rip.midind / samprate;
            rip.peak = tmprip(:,3);
            rip.energy = tmprip(:,7);
        ...
        rip.timerange = [0 length(renv)/samprate] + ripple{animal}{date}{f}.starttime;
        rip.samprate = ripple{animal}{date}{f}.samprate;
        ...
        %ripple
        clear feeg
        ripples{animal}{date}{f} = rip;
        clear rip; 
        ripfilename =  [directoryname, 'ripples', num2str(f)];
        save(ripfilename, 'ripples');
        clear ripples; %added ALP 4/23/19 to prevent writing multiple files to the same data structure
    end
end