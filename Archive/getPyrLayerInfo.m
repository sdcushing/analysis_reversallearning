...
randTime = [];
randInds = randi(length(rawDataBySessionNeural.vrTime),100000,1);
immobile = 0;
numsecs = 5;
t = 1;
while immobile == 0 && t<=length(randInds)
    randTime = randInds(t);
    if sum(rawDataBySessionNeural.speedSmooth(randTime:randTime+numsecs/0.02)) == 0 %samprate ~= 0.02 secs
        immobile = 1;
    else
        t = t+1;
    end
...
if immobile == 0
    sprintf('Random times did not produce 5 seconds of immobility.')
end
...
for ch = mych
    ...
    tmp_lfp = rawDataBySessionNeural.lfpData(ch,randTime:randTime+chparams.Fs*numsecs-1);
...
thetaPerCh = mean(all_ch_power(:,whichf),2);%mean power over this range of frequencies
...
for w = 1:size(data,2)-numChans+1
    ...
    allWThetaAmpMed(w) = median (thetaPerCh(thisW));
...
if plotPyrLayer
    ...
    ytickspacing = [];
    thisch = 0;
    figure; hold on; for ch = chans; thisch = thisch+1; plot(rawDataBySessionNeural.lfpData(ch,randTime:randTime+chparams.Fs*numsecs-1)+ch/10'); ytickspacing(thisch) = ch/10;end%can change devisor for spacing
