function [rawDataByTrial, statsByRewardTrial, statsByRewardSession] = getTrialBehaviorStats_linearJLK(rawDataBySession, virmen_fileInfo, params, saveBehaviorPath)
...
if length(lapStartIdx) <= 1 %no completed laps
    ...
    %save to rawDataByTrial and statsByRewardTrial structs
    rawDataByTrial = [];
    statsByRewardTrial = [];
    ...
    %determine number of zone types (only update track has 3: 1=reward, 2=nonreward, 3 = alt nonreward)
    if strcmp(virmen_fileInfo.trackname,"TrackA'") %update
        znTypeTot = 3;
    else %not update
        znTypeTot = 2;
    end

    for znType = 1:znTypeTot %1=reward, 2=nonreward, 3 = alt nonreward
        ...
        for znNum = 1:length(znStartDegs)
            if ~isempty(rawDataByTrial{znType,znNum})
                ...
                for tr = 1:length(rawDataByTrial{znType,znNum})
                    ...
                    velocCountsSmooth(tr,:) = smooth2005(velocCounts(tr,:),WinSize,'rlowess');
