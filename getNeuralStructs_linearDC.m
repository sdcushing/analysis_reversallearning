...
firstFile = fullfile(lfp_filepath, names{1}, 'raweeg.mat');
tmp = load(firstFile);
...
lfp_meta.nativechannel = zeros(numFolders,1);
...
for i = 1:numFolders
    ...
    matFile = fullfile(folderPath, 'raweeg.mat');

    S = load(matFile);
    R = S.raweeg;

    % Store LFP data
    lfp_data(i, :) = double(R.data);
