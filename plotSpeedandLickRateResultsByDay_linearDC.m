function plotSpeedandLickRateResultsByDay_linearDC(dirs,uniqSess,params, allindex)
...
for id = 1:2%:length(params.decoding.decID)
    ...
    if isfile(speedAllfname)
        ...
        if strcmp(params.decoding.decID{id}, 'lap')
            ...
            speedByGroup = nan(2,length(params.experimentalGroup),5,2,200,180);%assumes more experimental animals and max 5 days recording
            lickRateByGroup = nan(2,length(params.experimentalGroup),5,2,200,180);
            % 
            for g = 1:2
            %     tmpdat = [];
            %     if g == 1
            ...
            %         groupAnimalIDs = params.controlGroup;
            %     elseif g == 2
            %         tmpSpeedDat = speedAPOE4;
            %         tmpLickRateDat = lickRateAPOE4;
            %         groupAnimalIDs = params.experimentalGroup;
            %     end%if g
            %     for ee = 1:2
            %         tmpDayInds = [];
            %         find day indices
...
%%DC replacing the above with more robust method for reshaping the data
%%structure
                allAnimals = uniqSess(:,1);   % one animal ID per row of data
                if g == 1
                    groupAnimals = params.controlGroup;
                else
                    groupAnimals = params.experimentalGroup;
                end

                groupRows = find(ismember(allAnimals, groupAnimals));%mice belogning to this group
                mice = unique(allAnimals(groupRows), 'stable');%unique mice
                numMice = numel(mice);

                for m = 1:numMice
                    mouseID = mice(m);
                    mouseRows = find(allAnimals == mouseID);%rows for this mouse, mouserows(d) is row for day d
                    numDays = numel(mouseRows);
                    for d = 1:numDays
                        srcRow = mouseRows(d);
                        speedByGroup(g, m, d, :, :, :) = speedDatAll(srcRow, :, :, :);
                        lickRateByGroup(g, m, d, :, :, :) = lickRateDatAll(srcRow, :, :, :);
                    end%d
                    clear mouseRows
                end%m
                clear groupAnimals
        ...
        elseif strcmp(params.decoding.decID{id}, 'trial')
            ...
            speedByGroup = nan(2,length(params.experimentalGroup),5,2,3,400,50);%assumes more experimental animals and max 5 days recording
            lickRateByGroup = nan(2,length(params.experimentalGroup),5,2,3,400,50);
            allAnimals = uniqSess(:,1);
            for g = 1:2
                % if g == 1
                %     tmpSpeedDat = speedAPOE3;
                %     tmpLickRateDat = lickRateAPOE3;
                ...
                %%DC replacing above to be more robust
                if g == 1
                    groupAnimals = params.controlGroup;
                else
                    groupAnimals = params.experimentalGroup;
                end%g
                mice = unique(allAnimals(ismember(allAnimals, groupAnimals)), 'stable');
                %id mice in this group
                numMice = numel(mice);

                for m = 1:numMice
                    mouseID = mice(m);
                    mouseRows = find(allAnimals == mouseID);
                    numDays = numel(mouseRows);
                    for d = 1:numDays
                        srcRow = mouseRows(d);
                        %output = group x mouse x day x environment x zone type x trial x position bin
                        speedByGroup(g, m, d, :, :, :, :) = speedDatAll(srcRow, :, :, :, :);
                        lickByGroup(g, m, d, :, :, :, :) = lickRateDatAll(srcRow, :, :, :, :);
                    end%d
                    clear mouseRows
                end%m
                clear groupAnimals
