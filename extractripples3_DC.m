...
%
%position info - add functionality eventually
%
...
%check all .names here to make sure consistent with original filter
    for i = 1:size(ripple.data, 1)% loop through all channels
        ...
        if (thresh > 0) & any(find(renv<baseline))
            ...
            rip.starttime = ripple.starttime + rip.startind / samprate;
            rip.endtime = ripple.starttime + rip.endind / samprate;
            rip.midtime = ripple.starttime + rip.midind / samprate;
            rip.peak = tmprip(:,3);
            rip.energy = tmprip(:,7);
            rip.maxthresh = (tmprip(:,9) - baseline) / stdev;
            
            % position time element -  might want this functional at some
            % point
            if inclposinfo
        ...
        rip.timerange = [0 length(renv)/samprate] + ripple.starttime;
        rip.samprate = ripple.samprate;
