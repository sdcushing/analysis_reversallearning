function Kilosort4_Pipeline_intan_DC(subj, sessDate, processedDataPath)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%tmpFiles = ([processedDataPath]);% '\' sprintf('*%s_%s*',subj,sessDate)
tmpFiles = dir(fullfile(processedDataPath, 'intaninfo*'));
tmpFiles = fullfile(processedDataPath, {tmpFiles.name});
props = [];
for r = 1:size(tmpFiles,2)
    %read ap meta data
    curr_file = tmpFiles{r};
    load(curr_file);

    %read ap data
    %samp0 = 0;
    ap_samprate = (intaninfo.fileinfo.amplifier_sample_rate);%Hz
    nSamp = intaninfo.datapts;

    %save info
    props.fileNames{r} = curr_file;
    props.recLength(r) = nSamp;
    props.sampRate = ap_samprate;
    props.numChan = intaninfo.nAmpChannels;
    props.fileNums(r) = r;
end%r
...
numShanks = 2;
brainReg = {'CA3'};
for r = 1:size(props.fileNames,2)
    %create structure
...
%%%%% create memory map %%%%%
mmf = memmapfile([kilosort_path '\' dir([kilosort_path '\' 'allrecordings.bin']).name], 'Format', {dataType, [nChInFile nSamp], 'x'});
...
save([processedDataPath '\kilosort4\clusters.mat'], 'clusters','-v7.3')
save([processedDataPath '\kilosort4\clustermetrics.mat'], 'clustermetrics','-v7.3')
save([processedDataPath '\kilosort4\clusters_allrec.mat'], 'clusters_allrec','-v7.3')
