function [data] = getNeuralStructsByDay_linearDC(allindex,dirs,uniqSess,params)
    ...
    for ss = 1:length(uniqSess)%not sure why 12 here, figure out
        ...
        if ~isempty(sessionInfo)
            ...
            if ~exist(dayDatafname) || params.rewrite.decodingData
                ...
                lpCtr = zeros(3,1);
                lpRipCtr = zeros(4,150,20);%this is probably JK data format to match NP, need matching my 64 channel
                files = sessionInfo(:,3);
